<template>
    <!-- Main Navbar Background -->
    <div class="bg-[#1A0B2E] w-full min-w-full">
        <div class="flex flex-col gap-4 lg:gap-0 lg:flex-row lg:justify-between items-center text-white p-8 font-oswald max-w-[1440px] mx-auto">
            <!-- Email Section -->
            <div class="">
                <h1 class="text-xl">inquiry@fortnitedata.com</h1>
            </div>
            <!-- Font Awesome Icons & Login Button -->
            <div class="flex gap-10 items-center text-2xl">
                <i class="fa fa-search cursor-pointer "></i>
                <i class="fa fa-user cursor-pointer "></i>
                <button class="border-2 px-5 py-1 rounded-3xl cursor-pointer hover:text-[#DF3F2A] hover:shadow-[0_0_25px_2px] hover:rounded-full hover:shadow-[#DF3F2A] transition-all">
                <i class="fa fa-user"></i> Login
                </button>
            </div>
        </div>
    </div>
</template>
<script>

</script>