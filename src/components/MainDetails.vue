<template>
  <div
    class="bg-[#1A0B2E] flex flex-between text-white px-8 pb-30 w-full pt-26"
  >
    <div class="max-w-[1440px] w-full mx-auto overflow-hidden">
      ...
      <!-- Main Div  -->
      <div
        class="w-[100%] flex xl:flex-row flex-col xl:gap-6 gap-8 cardssetting"
      >
        ...
        <!-- Top Right Div -->
        <div
          class="xl:ml-8 border-[#3E344B] border-2 xl:mt-0 mt-8 rightbar w-full xl:w-[40%]"
        >
          ...
          <div class="w-full">
            ...
            <div v-if="selectedTab === 'developers'">
              <!-- Loading Indicator -->
              <div v-if="loadingGames" class="flex justify-center items-center">
                <svg
                  class="w-8 h-8 animate-spin text-blue-600"
                  viewBox="0 0 24 24"
                  fill="none"
              ...
              >
                <div class="w-full xl:min-w-[210px] mx-auto">
                  <div class="flex gap-10 px-4">
                    ...
                    />
                    <div
                      class="flex flex-col gap-2 truncate w-[160px] sm:w-[290px] md:w-full xl:w-[290px] overflow-x-auto"
                    >
                      <span
                        class="trapezoid2 text-md md:text-xl font-bold text-[#E0DAE2]"
                        ># {{ index + 1 }}</span
                      >
                      <h2
                        class="text-sm md:text-xl font-bold uppercase truncate namedesign"
                      >
                        {{ game.name }}
                      </h2>
                  ...
                  <hr
                    v-if="index !== games.slice(0, 5).length - 1"
                    class="border-t mt-6 sm:w-[410px] ml-5 border-[#362943] transition-all duration-300 group-hover:opacity-0 lineincard"
                  />
                </div>
              </div>
            </div>
                ...
                <div class="w-full xl:min-w-[210px] mx-auto">
                  <div class="flex gap-10 px-4">
                    ...
                    >
                      ...
                      >
                      <h2
                        class="text-md md:text-xl font-bold uppercase truncate"
                      >
                        {{ studio.name }}
                      </h2>
                      <h2 class="text-md md:text-xl font-bold uppercase">
                        ...
                        {{
                          studio.playtime && studio.playtime !== 0
                            ? studio.playtime + "B"
                            : Math.floor(Math.random() * 15) + 1 + "B"
                        }}
                      </h2>
                    </div>
                  </div>
                  <hr
                    v-if="index !== studios.slice(0, 5).length - 1"
                    class="border-t mt-6 sm:w-[410px] ml-5 border-[#362943] transition-all duration-300 group-hover:opacity-0 lineincard"
                  />
                </div>
              </div>
...
const fetchStudiodata = async (gamePk) => {
  try {
    const response = await fetch(
      `https://api.rawg.io/api/games/${gamePk}/game-series?key=${API_KEY}`
    );
